...
CLASSES = ["front_bumper", "font_hood", "rear_bumper", "side_door"]
IMG_SIZE = 150
...
def create_cnn_model_car_part() -> tf.keras.Model:
  ...
  model = tf.keras.Model(inputs=base_model.input, outputs=output_layer, name="chepo_cnn_model_car_part")
  model.summary()
  return model
...
def train() -> None:
  ...
  dataset = tf.keras.utils.get_file(
    os.path.abspath('./data/car_part_labelled_data.zip'),
    cache_subdir=os.path.abspath('.'),
    origin='',
    extract=True)

  dataset_path = os.path.abspath('.')+'/car_part_labelled_data/'
